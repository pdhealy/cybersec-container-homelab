...
if [ "${ACTIVE_METASPLOITABLE2:-false}" = "true" ]; then
    ...
    TARGET_IP="10.10.20.11" # We assume it's roughly here or the name resolves
...
function check_splunk_log {
    ...
    for i in $(seq 1 20); do
        if docker exec --user splunk splunk /opt/splunk/bin/splunk search "index=* earliest=-10m $query" -auth "admin:cyberhomelab_splunk_secure" 2>/dev/null | grep -vE "^WARNING:|^INFO:" | grep . >/dev/null; then
            return 0
        fi
        sleep 3
    done
